...
- (void)loadCaseDetailData {
    ...
    if (self.caseId.length || self.visitRecordIds.length) {
        [SVProgressHUD show];
        NSDictionary *paras = @{@"visitRecordIds" : self.visitRecordIds.length ? self.visitRecordIds : self.caseId};
        [[[WTCaseDetailViewModel alloc] init] dataByParameters:paras success:^(WTCaseDetailModel *detailModel) {
