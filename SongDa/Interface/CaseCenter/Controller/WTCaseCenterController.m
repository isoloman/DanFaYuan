...
#define kLocalName          @"长春市"
//#define kLocalName          @"昆明市"

@interface WTCaseCenterController () <UITableViewDelegate,UITableViewDataSource,WTCaseCenterCellDelegate>
/** 状态按钮 */
...
{
    if (_regionTableView == nil) {
        _regionTableView = [[UITableView alloc] initWithFrame:self.tableView.bounds];
        _regionTableView.height = HYXScreenH*0.5;
        _regionTableView.backgroundColor = [UIColor clearColor];
        _regionTableView.delegate = self;
        _regionTableView.dataSource = self;
        _regionTableView.bounces = NO;
        
        _regionTableView.estimatedRowHeight = 0;
        _regionTableView.estimatedSectionHeaderHeight = 0;
        _regionTableView.estimatedSectionFooterHeight = 0;
        adjustsScrollViewInsets(_regionTableView);
        
        [_regionTableView registerClass:[UITableViewCell class] forCellReuseIdentifier:regionID];
    }
...
- (void)setupNav {
    ...
    [zoneBtn setTitle:kLocalName forState:UIControlStateNormal];
...
- (void)zoneViewTransform {
    ...
    if (self.zoneBtn.selected) {
        ...
        [UIView animateWithDuration:0.2 animations:^{
            self.filterView.alpha = 0.3;
            self.blackBackView.alpha = 0.3;
            self.regionTableView.height = HYXScreenH*0.5;;
...
- (void)hideStatusView {
    ...
    [UIView animateWithDuration:0.2 animations:^{
        ...
        if (self.isExitReginView) {
            [self.regionTableView removeFromSuperview];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    if ([tableView isEqual:self.tableView]) {
        ...
        [self.tableView setContentOffset:CGPointZero animated:YES];
        WTCaseRegionModel *regionModel = self.regionTotalModel.data[indexPath.row];
        if ([regionModel.localName isEqualToString:kLocalName]) {
            self.regionId = @"";
        } else {
            self.regionId = regionModel.regionId;
        }
        [self.zoneBtn setTitle:regionModel.localName forState:UIControlStateNormal];
        [self loadNewData];
    }
